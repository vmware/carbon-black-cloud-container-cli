...
func (s *ScannedImage) CycloneDXDoc() []byte {
	doc := Document{
	...
	doc.BomDescriptor = NewBomDescriptor("cbctl", cbctlVersion.Version, s.FullTag, s.ManifestDigest)
	...
	for _, artifact := range s.Packages.Artifacts {
		...
		for _, vul := range s.Vulnerabilities {
			if artifact.Name == vul.Name {
				...
				ratings[0].Severity = vul.Severity
